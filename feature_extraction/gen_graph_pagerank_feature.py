...
def generate_qid_graph_table(row):
    hash_key1 = row["words1"]
    hash_key2 = row["words2"]
...
def get_pagerank_value(row):
    return pd.Series({
        "q1_word_pr": pagerank_dict[row["words1"]],
        "q2_word_pr": pagerank_dict[row["words2"]]
    })
...
pagerank_feats_train.to_csv(path + 'train_feature_graph_pagerank.csv', index=False)
pagerank_feats_test.to_csv(path + 'test_feature_graph_pagerank.csv', index=False)