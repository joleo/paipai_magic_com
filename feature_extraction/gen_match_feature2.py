# -*- coding: utf-8 -*-
# @Time    : 6/20/18 6:19 PM
# @Author  : Mat
# @Email   : mat_wu@163.com
# @File    : gen_match_feature2.py
...
#
# # 单词权重
# train_qs = pd.Series(train['words1'].tolist() + train['words2'].tolist() + train['chars1'].tolist() + train['chars2'].tolist()).astype(str)
...
#
# def word_shares(row):
#     q1_list = str(row['words1']).lower().split()
#     q1 = set(q1_list)
...
#
#     q2_list = str(row['words2']).lower().split()
#     q2 = set(q2_list)
...
# 平均单词长度、差值
df['len_char_q1'] = df['chars1'].apply(lambda x: len(str(x).split()))
df['len_char_q2'] = df['chars2'].apply(lambda x: len(str(x).split()))

df['len_word_q1'] = df['words1'].apply(lambda x: len(str(x).split()))
df['len_word_q2'] = df['words2'].apply(lambda x: len(str(x).split()))
...
df_feature[:train.shape[0]].to_csv('train_feature_match_2.csv', index=False)
df_feature[train.shape[0]:].to_csv('test_feature_match_2.csv', index=False)