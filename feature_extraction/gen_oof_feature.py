# -*- coding: utf-8 -*-
# @Time    : 6/20/18 6:56 PM
# @Author  : Mat
# @Email   : mat_wu@163.com
# @File    : gen_oof_feature.py
import matplotlib
import numpy as np
...
train = train[['label','words_x','words_y']].reset_index()
train.columns = ['id','label','question1','question2']
len_train = len(train)
...
df = pd.concat([train, test]).fillna('')
...
ix_is_dup = np.where(df['label'] == 1)[0]
ix_not_dup = np.where(df['label'] == 0)[0]
...
df['log_abs_diff_len1_len2'] = np.log(df['abs_diff_len1_len2'] + 1)  # 平滑处理
df['ratio_len1_len2'] = df['len1'].apply(lambda x: x if x > 0.0 else 1.0) / df['len2'].apply(
    lambda x: x if x > 0.0 else 1.0)
...
v_den = np.array(np.sqrt(m_q1_tf.multiply(m_q1_tf).sum(axis=1)))[:, 0] * np.array(
    np.sqrt(m_q2_tf.multiply(m_q2_tf).sum(axis=1)))[:, 0]
...
def func(w):
    return (mp * data['y_test_pred'].shape[0] -
            np.sum(w[0] * data['y_test_pred'] / (
                        w[0] * data['y_test_pred'] + (1 - w[0]) * (1 - data['y_test_pred'])))) ** 2
...
def func(w):
    return (
                   ...
                   np.sum(w[0] * data['y_test_pred'] / (
                               w[0] * data['y_test_pred'] + (1 - w[0]) * (1 - data['y_test_pred'])))
           ) ** 2
...
df[:len_train].to_csv('train_feature_oof.csv', index=False)
df[len_train:].to_csv('test_feature_oof.csv', index=False)
